...
def generate_final_video():
    ...
    for file in files:
        generate_cut_video(file, config['keyword'], config['seconds_to_cut'], config['useDebugFile'])

    generate_cut_videos = [os.path.join(VIDEO_GENERATION_PATH, file) for file in os.listdir(VIDEO_GENERATION_PATH) if os.path.isfile(os.path.join(VIDEO_GENERATION_PATH, file))]

    video_manipulation.merge_videos(generate_cut_videos, config["final_video_name"])
    end_time = dt.datetime.now()
    processing_time = (end_time - start_time).total_seconds() / 60
    generic_utils.remove_temp_dir()
    print(f"Finishing main process at {end_time}. Processing time: {processing_time:.2f} minutes with {len(files)} processed")